...
// Original file :
// https://github.com/loco-3d/crocoddyl/blob/devel/include/crocoddyl/core/solvers/ddp.hpp
//
 ...
 * \f$\mathbf{f}_s\f$ and run a modified Riccati sweep:
 * \f{eqnarray*}
 ...
 *   \mathbf{\hat{x}}_{k+1} &=&
 * \mathbf{f}_k(\mathbf{\hat{x}}_k,\mathbf{\hat{u}}_k).
 * \f}
 *
...
class SolverDDP : public crocoddyl::SolverAbstract {
 public:
   ...
   * infeasible guess through a modified Riccati sweep:
   * \f{eqnarray*}
   *   \mathbf{Q}_{\mathbf{x}_k} &=& \mathbf{l}_{\mathbf{x}_k} +
   * \mathbf{f}^\top_{\mathbf{x}_k} (V_{\mathbf{x}_{k+1}}
   * +
   ...
   * and
   * \f$\mathbf{f}_{\mathbf{u}_k}\f$ are the Jacobians of the cost function and
   * dynamics,
   ...
   * and feedback gains) by the `backwardPass()`:
   * \f{eqnarray}
   *   \mathbf{\hat{x}}_0 &=& \mathbf{\tilde{x}}_0,\\
   *   \mathbf{\hat{u}}_k &=& \mathbf{u}_k + \alpha\mathbf{k}_k +
   * \mathbf{K}_k(\mathbf{\hat{x}}_k-\mathbf{x}_k),\\
   ...
   * \f$\mathbf{Q}_{\mathbf{uu}_k}^{-1}\f$ term:
   * \f{eqnarray}
