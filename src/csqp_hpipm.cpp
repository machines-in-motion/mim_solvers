...
SolverHPIPM::SolverHPIPM(boost::shared_ptr<crocoddyl::ShootingProblem> problem)
    : SolverDDP(problem){      
...
SolverHPIPM::~SolverHPIPM() {}

bool SolverHPIPM::solve(const std::vector<Eigen::VectorXd>& init_xs, const std::vector<Eigen::VectorXd>& init_us,
                       const std::size_t maxiter, const bool is_feasible, const double reginit) {
  
  (void)is_feasible;
  (void)reginit;

  START_PROFILER("SolverHPIPM::solve");
  ...
  for (iter_ = 0; iter_ < maxiter; ++iter_) {
    ...
    if(use_kkt_criteria_){
      if (KKT_  <= termination_tol_) {
        STOP_PROFILER("SolverHPIPM::solve");
        return true;
      }
    }
...
void SolverHPIPM::calc(const bool recalc){
  if (recalc){
...
void SolverHPIPM::computeDirection(const bool recalcDiff){
  START_PROFILER("SolverHPIPM::computeDirection");
  const std::size_t T = problem_->get_T();
  if (recalcDiff) {
    calc(recalcDiff);
  }
  ...
  STOP_PROFILER("SolverHPIPM::computeDirection");
}

void SolverHPIPM::checkKKTConditions(){
  const std::size_t T = problem_->get_T();
  const std::size_t ndx = problem_->get_ndx();
  const std::vector<boost::shared_ptr<ActionDataAbstract> >& datas = problem_->get_runningDatas();
  KKT_ = 0;
...
double SolverHPIPM::tryStep(const double steplength) {
    if (steplength > 1. || steplength < 0.) {
        throw_pretty("Invalid argument: "
                    << "invalid step length, value is between 0. to 1.");
    }

    START_PROFILER("SolverHPIPM::tryStep");
    cost_try_ = 0.;
    merit_try_ = 0;
    gap_norm_try_ = 0;
    ...
    for (std::size_t t = 0; t < T; ++t) {
      ...
      if (raiseIfNaN(cost_try_)) {
        STOP_PROFILER("SolverHPIPM::tryStep");
...
// const Eigen::MatrixXd& SolverHPIPM::get_P() const {return P_;};


void SolverHPIPM::printCallbacks(){
...
void SolverHPIPM::setCallbacks(bool inCallbacks){
  with_callbacks_ = inCallbacks;
}

bool SolverHPIPM::getCallbacks(){
